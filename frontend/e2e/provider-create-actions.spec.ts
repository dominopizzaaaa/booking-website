...
async function createBookingThroughProviderUi(page: Page, journey: ProviderJourney) {
  ...
  await dialog.getByLabel('Class', { exact: true }).selectOption(journey.serviceId);
  ...
  const venueNotice = dialog.getByText(
    'This class will be pending venue confirmation. Scheduling does not reserve an external court or room.',
    { exact: true },
  );
...
test('a club creates a booking and completes its venue, receipt, reversal, and cancellation actions', async ({ page }, testInfo) => {
  ...
  await expect(dialog.getByText(
    'Please secure the venue separately, then confirm this class. Courtly has not reserved the court.',
    { exact: true },
  )).toBeVisible();
...
test('a coach account cannot create or enter a private practice', async ({ page }, testInfo) => {
  const runId = uniqueValue('retired-practice', testInfo.project.name);
  const coachName = `Club-only Coach ${projectId(testInfo.project.name)}`;
  const coachEmail = `${runId}-coach@example.test`;
  ...
  await expect(page.getByLabel('Name your practice', { exact: true })).toHaveCount(0);
  await expect(page.getByRole('button', { name: 'Create practice', exact: true })).toHaveCount(0);
  await expect(page.locator('main')).not.toContainText('Own practice');

  const before = await responseJson<AuthSession>(await page.request.get('/api/auth/me'));
  expect(before.business).toBeNull();
  expect(before.membership).toBeNull();
