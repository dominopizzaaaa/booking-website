...
import type { AccountType, AuthSession, BusinessKind } from '@/lib/types';
...
// How money moves decides a great deal downstream, so it is asked once, at the
// point the workspace is created, rather than inferred later.
const businessKinds: Array<{ value: BusinessKind; label: string; detail: string }> = [
  { value: 'CLUB', label: 'Club or academy', detail: 'Students pay the club. The club pays its coaches.' },
  { value: 'SOLO', label: 'Independent coach', detail: 'Your students pay you directly.' },
];
...
function isCustomerDestination(destination: string | null) {
...
function destinationFor(state: AuthSession, requested: string | null) {
  ...
  if (state.user.accountType === 'CUSTOMER') return '/manage';
...
export function AuthForm({ mode }: { mode: 'login' | 'signup' }) {
  ...
  const [businessKind, setBusinessKind] = useState<BusinessKind>('CLUB');
  ...
  useEffect(() => {
    ...
    if (signup) setAccountType(isCustomerDestination(requested?.destination ?? null) ? 'CUSTOMER' : null);
  }, [signup]);
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const result = await api<AuthSession>(signup ? '/auth/register' : '/auth/login', { method: 'POST', body: JSON.stringify(signup ? { accountType, ...(accountType === 'OWNER' ? { businessName: values.businessName.trim(), businessKind } : {}), name: values.name.trim(), email: values.email.trim(), password: values.password } : { email: values.email.trim(), password: values.password }) });
    ...
    <section className="flex min-h-screen flex-col px-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-4 sm:px-12 sm:py-9 lg:px-14 xl:px-24">
      ...
      <div className="mx-auto my-auto w-full max-w-[390px] py-8 sm:py-16">
        ...
        <form className="!mt-7 space-y-4 sm:!mt-8 sm:space-y-5" onSubmit={submit}>
          ...
          <button type="submit" className={primary} disabled={!!busy}>{busy === 'form' ? <><LoaderCircle size={16} className="animate-spin" />{signup ? 'Creating your account…' : 'Signing you in…'}</> : <>{signup ? accountType === 'OWNER' ? 'Create your workspace' : accountType === 'COACH' ? 'Create coach account' : accountType === 'CUSTOMER' ? 'Create player account' : 'Create account' : 'Sign in'}<ArrowRight size={16} /></>}</button>
        </form>
