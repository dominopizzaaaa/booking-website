...
export function AuthForm({ mode }: { mode: 'login' | 'signup' }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (busy) return;
    if (signup && (values.name.trim().length < 2 || values.businessName.trim().length < 2)) { setError('Please enter your name and business name using at least two characters each.'); return; }
    setBusy('form'); setError('');
    try {
      await api(signup ? '/auth/register' : '/auth/login', { method: 'POST', body: JSON.stringify(signup ? { businessName: values.businessName.trim(), name: values.name.trim(), email: values.email.trim(), password: values.password } : { email: values.email.trim(), password: values.password }) });
  ...
  return <main className="min-h-screen overflow-x-clip bg-[#f6f7f4] text-[#1c3029] lg:grid lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
    <section className="relative hidden min-h-screen overflow-hidden bg-[#174c3c] p-12 text-white lg:flex lg:flex-col xl:p-16">
      ...
      <div className="relative z-10 my-auto max-w-lg py-14">
        ...
        <p className="!mt-6 max-w-sm text-sm leading-[1.9] text-[#a8c0af]">Your lessons, your people, your places. Beautifully in sync — so you can get back to what you do best.</p>
    ...
    <section className="flex min-h-screen flex-col px-5 pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-4 sm:px-12 sm:py-9 lg:px-14 xl:px-24">
      ...
      <div className="mx-auto my-auto w-full max-w-[390px] py-8 sm:py-16">
        ...
        <p className="!mb-2 text-[10px] font-semibold uppercase tracking-[2px] text-[#95a085]">{signup ? 'A FRESH START, A LITTLE MORE PLAY' : 'YOUR COACHING DAY, IN SYNC'}</p>
        ...
        <p className="!mt-3 text-sm leading-relaxed text-[#8a957f]">{signup ? 'A calmer way to run your coaching business starts here. Let’s make it yours.' : 'A new day, a fresh start. Your workspace is ready when you are.'}</p>
        <form className="!mt-7 space-y-4 sm:!mt-8 sm:space-y-5" onSubmit={submit}>
          {signup && <><div><label htmlFor="auth-business" className="!mb-2 !text-xs !font-medium !text-[#617257]">Business name</label><input id="auth-business" className={input} value={values.businessName} onChange={event => update('businessName', event.target.value)} required minLength={2} maxLength={120} autoComplete="organization" placeholder="e.g. Oakwood Tennis Academy" disabled={!!busy} /></div><div><label htmlFor="auth-name" className="!mb-2 !text-xs !font-medium !text-[#617257]">Your full name</label><input id="auth-name" className={input} value={values.name} onChange={event => update('name', event.target.value)} required minLength={2} maxLength={120} autoComplete="name" placeholder="e.g. Jamie Lee" disabled={!!busy} /></div></>}
          ...
          <button type="submit" className={primary} disabled={!!busy}>{busy === 'form' ? <><LoaderCircle size={16} className="animate-spin" />{signup ? 'Creating your workspace…' : 'Opening your workspace…'}</> : <>{signup ? 'Create your workspace' : 'Sign in to your workspace'}<ArrowRight size={16} /></>}</button>
        </form>
        <div className="my-5 flex items-center gap-3 sm:my-6 sm:gap-4"><span className="h-px flex-1 bg-[#e3e7dd]" /><span className="shrink-0 text-[10px] text-[#a1aa94]">or take a little look around</span><span className="h-px flex-1 bg-[#e3e7dd]" /></div>
        ...
        <p className="!mt-3 text-center text-[10px] leading-relaxed text-[#9ca68e]">No sign-up needed. A sample coaching business, ready to explore.</p>
        {signup && <div className="!mt-7 flex items-start gap-2.5 rounded-xl border border-[#e5e9df] bg-white/50 p-3.5 text-[10px] leading-relaxed text-[#8d9b7d]"><ShieldCheck size={14} className="!mt-0.5 shrink-0" />Your workspace is private. Set up your services, coaches, and locations, then share your public booking page when you’re ready.</div>}
      </div>
