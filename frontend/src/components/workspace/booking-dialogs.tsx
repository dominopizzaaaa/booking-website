...
export function NewBookingDialog({ data, open, onClose, refresh }: { data: Workspace; open: boolean; onClose: () => void; refresh: () => Promise<void> }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault(); if (!slot || !customerId) return;
    setBusy(true); setError('');
    try {
      const form = new FormData(event.currentTarget);
      await createBooking({ serviceId, locationId, instructorId, startAt: slot, customerId, repeatWeeks: repeat, ...(packageId ? { packageId } : {}), notes: String(form.get('notes') || ''), address: String(form.get('address') || '') });
  ...
  return <Dialog open={open} onOpenChange={v => { if (!v) onClose(); }}><DialogContent className="max-w-xl"><DialogTitle className="text-xl font-semibold tracking-tight">Add a booking</DialogTitle><DialogDescription className="mt-2 mb-6 text-xs text-stone-500">A quick booking for the lessons arranged off the court.</DialogDescription><form onSubmit={submit} className="form-stack">
    ...
    <div><label htmlFor="booking-customer">Registered customer</label><select id="booking-customer" value={customerId} onChange={e => { setCustomer(e.target.value); setPackage(''); }} required><option value="" disabled>Select a customer account</option>{data.customers.filter(c => !!c.userId).map(c => <option key={c.id} value={c.id}>{c.name} · {c.email}</option>)}</select><p className="mt-1 text-[10px] leading-relaxed text-stone-400">Bookings can only be created for customers already linked to a Courtly customer account. Add them from Customers first if they are missing.</p></div>
    {canSeeFinancials && customerId && data.packages.some(p => p.customerId === customerId && p.usedCredits < p.totalCredits) && <div><label htmlFor="booking-package">Lesson package</label><select id="booking-package" value={packageId} onChange={e => setPackage(e.target.value)}><option value="">Pay per session</option>{data.packages.filter(p => p.customerId === customerId && p.usedCredits < p.totalCredits && (!p.serviceId || p.serviceId === serviceId)).map(p => <option key={p.id} value={p.id}>{p.name} · {p.totalCredits - p.usedCredits} credits left</option>)}</select></div>}
    ...
    <div className="flex items-center justify-between gap-4 border-t border-stone-100 pt-5"><div>{canSeeFinancials ? <><p className="text-lg font-semibold">{packageId ? `${repeat} package credit${repeat > 1 ? 's' : ''}` : money((mapping?.price ?? service?.price ?? 0) * repeat)}</p><p className="mt-1 text-[10px] text-stone-500">{repeat} lesson{repeat > 1 ? 's' : ''} · no payment collected online</p></> : <p className="text-[11px] leading-relaxed text-stone-500">{repeat} lesson{repeat > 1 ? 's' : ''} · payment details stay with an owner or administrator</p>}</div><Button type="submit" disabled={busy || !slot || !customerId}>{busy ? <Loader2 size={15} className="animate-spin" /> : <Check size={15} />}Add booking</Button></div>
