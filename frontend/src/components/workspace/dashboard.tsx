...
export function ScheduleList({ bookings, onSelect, data, compact = false }: { bookings: WorkspaceBooking[]; onSelect: (bookingId: string) => void; data: WorkspaceResponse; compact?: boolean }) {
  ...
  return <div className={`schedule-list ${compact ? '!px-4 !py-3 md:max-h-[430px] md:overflow-y-auto' : ''}`}>{!sorted.length && <div className={`empty-state ${compact ? '!py-12' : ''}`}><CalendarDays size={27} /><h3>Your day has a little breathing room.</h3><p>No lessons scheduled for this date.</p></div>}{sorted.map((b, index) => {
    ...
    return <div key={b.id}><div className={`schedule-row ${compact ? '!mb-2 !gap-3' : ''}`}><div className={`schedule-time ${compact ? '!basis-12 !pt-2.5 !text-[11px]' : ''}`}>{formatInTimeZone(b.startAt, data.business.timezone, 'h:mm')}<span className="ml-1 text-[9px]">{formatInTimeZone(b.startAt, data.business.timezone, 'a').toLowerCase()}</span></div><button className={`lesson-card ${['', 'blue', 'amber', 'purple'][Math.max(0, locationIndex) % 4]} ${compact ? '!rounded-lg !px-3.5 !py-2.5 max-sm:!grid max-sm:!grid-cols-[minmax(0,1fr)_auto] max-sm:!items-start max-sm:!gap-2' : ''}`} onClick={() => onSelect(b.id)}><div className="min-w-0"><div className="flex min-w-0 items-start gap-2"><p className="lesson-title min-w-0 break-words text-left leading-snug">{b.serviceName}</p>{b.recurringId && <Repeat2 size={11} className="mt-0.5 shrink-0 text-[#a2ac96]" />}</div><div className={`lesson-meta ${compact ? '!mt-1.5 !text-[11px]' : ''}`}><span className="min-w-0 break-words">{b.participants.length === 1 ? b.participants[0].name : `${b.participants.length} students`}</span><span className="text-[#c5ccba]">·</span><span className="flex min-w-0 items-center gap-1"><MapPin size={10} className="shrink-0" /><span className="truncate">{b.locationName}</span></span><span className="desktop-only text-[#c5ccba]">·</span><span className="desktop-only">{Math.round((+new Date(b.endAt) - +new Date(b.startAt)) / 60000)} min</span></div></div><div className="flex shrink-0 flex-col items-end gap-2 max-sm:gap-1.5"><StatusBadge status={b.status} coachAcceptance={b.coachAcceptance} className={compact ? '!text-[10px] max-sm:!px-1.5' : ''} /><div className="flex items-center gap-1.5"><span className="text-[10px] text-[#8f9988] max-sm:hidden">{b.instructorName.split(' ')[0]}</span><div className="tiny-avatar">{initials(b.instructorName)}</div></div></div></button></div>{travel && buffer > 0 && <div className={`travel-row ${compact ? '!mb-2 !ml-[60px] !text-[10px]' : ''}`}><Route size={12} /><span>{buffer} min travel buffer</span><span>→</span><span>{next.locationName}</span></div>}</div>;
  })}</div>;
}
...
export default function Dashboard({ data, onNavigate, onNew, onBooking }: { data: WorkspaceResponse; onNavigate: (v: string) => void; onNew: () => void; onBooking: (bookingId: string) => void }) {
  ...
  const visibleLocations = isClubCoach ? data.locations.filter(location => assignedLocationIds.has(location.id)) : data.locations;
  ...
  return <div className="mx-auto max-w-[1380px]">
    ...
    <section aria-label={isCoach ? 'Coach snapshot' : 'Business snapshot'} className="mt-5 grid grid-cols-2 gap-px overflow-hidden rounded-xl border border-[#e2e7df] bg-[#e2e7df] shadow-[0_1px_2px_rgba(25,52,38,0.02)] md:grid-cols-4">
      ...
      {isClubCoach ? <Stat title="This week's sessions" value={String(weekBookings.length).padStart(2, '0')} detail={`${weekHours % 1 ? weekHours.toFixed(1) : weekHours} teaching hours`} icon={<Clock3 size={16} />} /> : <Stat title="This week's earnings" value={money(weeklyRevenue)} detail="Payments collected" icon={<Wallet size={16} />} />}
      <Stat title={isClubCoach ? 'Assigned students' : 'Students'} value={String(data.students.length).padStart(2, '0')} detail={`${uniqueToday} booked today`} icon={<Users size={16} />} />
      {isClubCoach ? <Stat title="Availability" value={String(data.availability.length).padStart(2, '0')} detail="Weekly teaching windows" icon={<CalendarDays size={16} />} onClick={() => onNavigate('availability')} /> : <Stat title="Outstanding" value={money(outstanding)} detail={`${outstandingCount} unpaid session${outstandingCount === 1 ? '' : 's'}`} icon={<CreditCard size={16} />} onClick={() => onNavigate('payments')} />}
    </section>
    ...
    <div className="mt-5 grid items-start gap-5 lg:grid-cols-[minmax(0,1.65fr)_minmax(280px,0.78fr)]">
      ...
      <aside className="grid gap-4 md:grid-cols-2 lg:grid-cols-1">
        ...
        <section className="panel overflow-hidden" aria-labelledby="up-next-heading">
          ...
          <div className="border-t border-[#edf0e8]">{recent.slice(0, 3).map(b => { const managerBooking = managerData?.bookings.find(booking => booking.id === b.id); const paid = managerBooking?.participants.every(p => p.paid || p.packageId); return <button key={b.id} className="flex w-full items-center gap-3 border-b border-[#f0f2ed] px-5 py-3 text-left last:border-b-0 hover:bg-[#fafbf8]" onClick={() => onBooking(b.id)}><div className="tiny-avatar !h-8 !w-8 !min-w-8 !bg-[#f0e9dc] !text-[9px] !text-[#8e7958]">{initials(b.participants[0]?.name || 'Group')}</div><div className="min-w-0 flex-1"><p className="truncate text-[12px] font-semibold">{b.serviceName}</p><p className="mt-1 truncate text-[10px] text-[#899286]">{b.participants[0]?.name || 'Group lesson'} · {formatInTimeZone(b.startAt, data.business.timezone, 'EEE, h:mm a')}</p></div>{managerBooking && <span className={`badge ${paid ? '' : 'pending'} !text-[9px]`}>{paid ? 'Paid' : 'Unpaid'}</span>}</button>; })}{!recent.length && <div className="empty-state !py-8">Your next great lesson starts with a booking.</div>}</div>
        </section>
        ...
        <section className="panel overflow-hidden md:col-span-2 lg:col-span-1" aria-labelledby="week-heading">
          ...
          <div className="flex items-end gap-4 px-5 pb-4"><div className="shrink-0 pb-1"><p className="text-[21px] font-semibold tracking-[-0.5px]">{weekBookings.length}</p><p className="mt-0.5 text-[10px] text-[#8d9788]">sessions booked</p></div><div className="flex h-14 flex-1 items-end gap-1.5">{(isClubCoach ? sessionDays : revenueDays).map((amount, i) => <div key={i} className="flex h-full flex-1 flex-col items-center justify-end gap-1"><div className={`w-full max-w-8 rounded-t-sm ${i === dayOfWeek - 1 ? 'bg-[#55794f]' : 'bg-[#e3eadb]'}`} style={{ height: `${Math.max(3, amount / (isClubCoach ? maxSessions : maxRevenue) * 38)}px` }} title={isClubCoach ? `${['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'][i]}: ${amount} session${amount === 1 ? '' : 's'}` : `${['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'][i]}: ${money(amount)}`} /><span className="text-[8px] text-[#9aa293]">{['M', 'T', 'W', 'T', 'F', 'S', 'S'][i]}</span></div>)}</div></div>
          <div className="flex items-center justify-between border-t border-[#edf0e8] px-5 py-3"><span className="text-[10px] text-[#8d9788]">{isClubCoach ? 'Teaching time this week' : 'Collected this week'}</span><span className="text-[12px] font-semibold">{isClubCoach ? `${weekHours % 1 ? weekHours.toFixed(1) : weekHours} hours` : money(weeklyRevenue)}</span></div>
        </section>
    ...
    <footer className="mt-5 flex items-center justify-between text-[10px] text-[#9ea699]"><span className="flex items-center gap-1.5"><span className="h-1 w-1 rounded-full bg-[#9faf87]" />All times in {data.business.timezone}{isClubCoach ? '' : ` · ${data.business.currency}`}</span><span className="desktop-only">More time for the game. That’s Courtly.</span></footer>
  </div>;
}
