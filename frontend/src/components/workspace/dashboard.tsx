...
export default function Dashboard({ data, onNavigate, onNew, onBooking }: { data: WorkspaceResponse; onNavigate: (v: string) => void; onNew: () => void; onBooking: (bookingId: string) => void }) {
  ...
  return <div className="mx-auto max-w-[1380px]">
    ...
    <footer className="mt-5 flex items-center justify-between text-[10px] text-[#59675c]"><span className="flex items-center gap-1.5"><span className="h-1 w-1 rounded-full bg-[#9faf87]" />All times in {data.business.timezone}{isClubCoach ? '' : ` · ${data.business.currency}`}</span><span className="desktop-only">More time for the game. That’s Courtly.</span></footer>
  </div>;
}
function Stat({ title, value, detail, icon, onClick }: { title: string; value: string; detail: string; icon: React.ReactNode; onClick?: () => void }) {
  const content = <><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-[11px] font-medium text-[#59675c]">{title}</p><p className="mt-1.5 truncate text-[23px] font-semibold tracking-[-0.6px] text-[#263a30]">{value}</p></div><span className="stat-icon !h-8 !w-8 shrink-0">{icon}</span></div><div className="mt-1.5 flex items-center justify-between gap-2"><p className="truncate text-[10px] text-[#59675c]">{detail}</p>{onClick && <ArrowUpRight size={12} className="shrink-0 text-[#829076]" />}</div></>;
