...
export function StudentsView({ data, refresh }: WorkspaceProps) {
  ...
  function creditBalance(studentId: string) {
    return managerData?.packages
      .filter(pkg => pkg.studentId === studentId && purchasedPackageState(pkg) === 'ACTIVE')
      .reduce((sum, pkg) => sum + Math.max(0, pkg.totalCredits - pkg.usedCredits), 0) ?? 0;
  }

  return <>
    <PageHeading title="Students" description="The people at the heart of your business. Remember the details that make a class personal." action={canManage ? () => setEditing(null) : undefined} actionLabel="Add student" />
    <section className="panel overflow-hidden" aria-labelledby="student-directory-title">
      <div className="panel-heading flex-wrap border-b border-[#edf0e8]">
        ...
        <div className="relative w-full sm:max-w-80"><Search size={14} className="pointer-events-none absolute left-3 top-3.5 text-[#59675c]" /><input type="search" aria-label="Search students by name, email, phone, or parent" placeholder="Search name, email, or phone…" value={query} onChange={event => setQuery(event.target.value)} className="pl-9! text-xs!" /></div>
      </div>
      {filtered.length ? <>
        <div className="table-wrap max-sm:hidden">
          <table className="data-table"><thead><tr><th>Student</th><th>Contact</th><th>Bookings</th><th>Latest scheduled</th>{canManage && <th>Credits left</th>}<th><span className="sr-only">View profile</span></th></tr></thead><tbody>{filtered.map(student => {
            ...
            return <tr key={student.id} className="transition-colors hover:bg-[#fafcf7]"><td><button className="flex items-center gap-3 text-left" onClick={() => setProfile(student.id)} aria-label={`View ${student.name}'s profile`}><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-[#ecf1e4] text-[10px] font-semibold text-[#56654f]">{student.initials || initials(student.name)}</span><span><span className="block font-medium">{student.name}</span>{student.parentName && <span className="mt-1 block text-[10px] text-[#59675c]">Parent: {student.parentName}</span>}</span></button></td><td><p>{student.email}</p><p className="mt-1 text-[10px] text-[#59675c]">{student.phone || 'No phone added'}</p></td><td>{student.bookingCount}</td><td>{student.lastBookingAt ? shortDate(student.lastBookingAt) : 'No bookings yet'}</td>{canManage && <td>{credits ? <span className="badge">{credits} credits</span> : <span className="text-[#59675c]">—</span>}</td>}<td><Button variant="ghost" size="icon" onClick={() => setProfile(student.id)} aria-label={`View ${student.name}'s profile`}><ArrowUpRight size={14} /></Button></td></tr>;
          })}</tbody></table>
        </div>
        <div className="divide-y divide-[#edf0e8] sm:hidden">
          {filtered.map(student => {
            const credits = creditBalance(student.id);
            return <button key={student.id} type="button" onClick={() => setProfile(student.id)} className="flex w-full items-start gap-3 p-4 text-left transition-colors active:bg-[#f6f8f2]" aria-label={`View ${student.name}'s profile`}><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#ecf1e4] text-xs font-semibold text-[#56654f]">{student.initials || initials(student.name)}</span><span className="min-w-0 flex-1"><span className="flex items-start justify-between gap-2"><span className="min-w-0"><span className="block truncate text-sm font-semibold text-[#294735]">{student.name}</span><span className="mt-1 block truncate text-[11px] text-stone-500">{student.email}</span></span><ArrowUpRight size={15} className="mt-0.5 shrink-0 text-[#59675c]" /></span><span className="mt-3 flex flex-wrap gap-2 text-[10px] text-[#59675c]"><span className="rounded-full bg-[#f4f6f1] px-2 py-1">{student.bookingCount} booking{student.bookingCount === 1 ? '' : 's'}</span>{canManage && credits > 0 && <span className="rounded-full bg-[#edf2e8] px-2 py-1 text-[#56654f]">{credits} credits</span>}<span className="rounded-full bg-[#f4f6f1] px-2 py-1">{student.lastBookingAt ? shortDate(student.lastBookingAt) : 'No classes yet'}</span></span></span></button>;
          })}
        </div>
...
function ParticipantFinancialSummary({ data, bookingId, studentId }: { data: ManagerWorkspace; bookingId: string; studentId: string }) {
  ...
  return <><span>·</span><span>{participant.packageId ? 'Package credit' : participant.paid ? `${money(participant.price, data.business.currency)} paid` : `${money(participant.price, data.business.currency)} unpaid`}</span></>;
}

function StudentPackageSummary({ pkg, data }: { pkg: ManagerWorkspace['packages'][number]; data: ManagerWorkspace }) {
  const state = purchasedPackageState(pkg);
  const scopes = purchasedPackageScopes(pkg, data.services, data.locations);
  const remaining = Math.max(0, pkg.totalCredits - pkg.usedCredits);
  const status = state === 'UNPAID' ? 'Payment due' : state === 'EXPIRED' ? 'Expired' : state === 'EXHAUSTED' ? 'Used up' : 'Active';
  const badgeClass = state === 'UNPAID' ? 'pending' : state === 'EXPIRED' ? 'cancelled' : state === 'EXHAUSTED' ? 'completed' : '';

  return <article className="rounded-xl border border-[#e6eae3] p-4"><div className="flex flex-wrap items-center justify-between gap-2"><h3 className="text-xs text-[#344b39]">{pkg.name}</h3><span className={`badge ${badgeClass}`}>{status}</span></div><dl className="mt-3 space-y-1.5 text-[10px] text-stone-500">{scopes.classNames.length > 0 && <div><dt className="inline font-semibold text-[#405941]">Classes: </dt><dd className="inline">{scopes.classNames.join(', ')}</dd></div>}{scopes.rentalNames.length > 0 && <div><dt className="inline font-semibold text-[#405941]">Rentals: </dt><dd className="inline">{scopes.rentalNames.join(', ')}</dd></div>}</dl><p className="mt-3 text-[11px] text-stone-500">{remaining} credits left · {pkg.usedCredits} of {pkg.totalCredits} used · {money(pkg.price, data.business.currency)}</p><p className="mt-1 text-[10px] text-[#59675c]">{state === 'EXPIRED' ? 'Expired' : 'Expires'} {shortDate(pkg.expiresAt, data.business.timezone)} {dateKey(pkg.expiresAt, data.business.timezone).slice(0, 4)}</p></article>;
}
...
function StudentProfile({ student, data, onClose, onEdit, onPackage, onPayment }: Omit<WorkspaceProps, 'refresh'> & { student: Student; onClose: () => void; onEdit: () => void; onPackage: () => void; onPayment: () => void }) {
  const [tab, setTab] = useState<'history' | 'packages'>('history');
  const historyTabRef = useRef<HTMLButtonElement>(null);
  const packagesTabRef = useRef<HTMLButtonElement>(null);
  const managerData = isManagerWorkspace(data) ? data : null;
  ...
  const recordTabs: Array<'history' | 'packages'> = managerData ? ['history', 'packages'] : ['history'];

  function handleTabKeyDown(event: KeyboardEvent<HTMLButtonElement>, current: 'history' | 'packages') {
    const currentIndex = recordTabs.indexOf(current);
    ...
    const next = recordTabs[nextIndex];
    setTab(next);
    (next === 'history' ? historyTabRef : packagesTabRef).current?.focus();
  }

  return <Dialog open onOpenChange={open => { if (!open) onClose(); }}><DialogContent className="max-w-2xl p-0 max-sm:top-auto max-sm:bottom-0 max-sm:max-h-[92dvh] max-sm:w-full max-sm:translate-y-0 max-sm:rounded-b-none"><div className="px-5 py-5 sm:px-6 sm:py-6"><div className="flex items-center gap-4 pr-6"><span className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-[#eaf0df] text-lg font-medium text-[#56654f]">{student.initials || initials(student.name)}</span><div className="min-w-0"><DialogTitle className="truncate text-xl font-semibold tracking-tight text-[#173f2f]">{student.name}</DialogTitle><DialogDescription className="mt-1 text-xs text-stone-500">Student since {shortDate(student.createdAt)} {dateKey(student.createdAt).slice(0, 4)}</DialogDescription></div></div><div className="mt-5 grid gap-2 text-xs text-stone-500 sm:grid-cols-2"><p className="flex items-center gap-2 break-all"><Mail size={13} className="shrink-0" />{student.email}</p><p className="flex items-center gap-2"><Phone size={13} className="shrink-0" />{student.phone || 'No phone added'}</p>{student.parentName && <p className="flex items-center gap-2 sm:col-span-2"><UserRound size={13} className="shrink-0" />Parent / guardian: {student.parentName}</p>}</div><dl className={`my-5 grid gap-2 rounded-xl bg-[#f4f7ef] p-3 sm:gap-3 sm:p-4 ${managerData ? 'grid-cols-3' : 'grid-cols-1'}`}><div className="min-w-0"><dt className="text-[9px] leading-tight text-[#59675c] sm:text-[10px]">Bookings</dt><dd className="mt-1 text-lg font-semibold text-[#254b38] sm:text-xl">{student.bookingCount}</dd></div>{managerData && <><div className="min-w-0"><dt className="text-[9px] leading-tight text-[#59675c] sm:text-[10px]">Payments recorded</dt><dd className="mt-1 break-words text-sm font-semibold text-[#254b38] sm:text-xl">{money(payments.reduce((sum, payment) => sum + payment.amount, 0), data.business.currency)}</dd></div><div className="min-w-0"><dt className="text-[9px] leading-tight text-[#59675c] sm:text-[10px]">Balance due</dt><dd className="mt-1 break-words text-sm font-semibold text-[#254b38] sm:text-xl">{money(outstanding, data.business.currency)}</dd></div></>}</dl>{managerData && <div className="grid grid-cols-1 gap-2 sm:flex sm:flex-wrap"><Button size="sm" variant="outline" onClick={onEdit}><Pencil size={13} />{student.userId ? 'Edit internal notes' : 'Edit profile & notes'}</Button><Button size="sm" variant="outline" onClick={onPackage}><Package size={13} />Sell package</Button><Button size="sm" variant="outline" onClick={onPayment}><Wallet size={13} />Record payment</Button></div>}<section className="mt-5 rounded-xl border border-[#e6eae3] bg-[#fbfcfa] p-4" aria-labelledby="student-notes-title"><h3 id="student-notes-title" className="text-xs text-[#344b39]">Internal notes</h3><p className="mt-2 whitespace-pre-wrap text-xs leading-relaxed text-stone-500">{student.notes || 'No notes yet. Add goals or preferences to make their next class feel personal.'}</p></section><div className="tab-bar my-5 w-full" role="tablist" aria-label="Student records"><button ref={historyTabRef} id="student-history-tab" type="button" role="tab" tabIndex={tab === 'history' ? 0 : -1} aria-selected={tab === 'history'} aria-controls="student-history-panel" className={`flex-1 ${tab === 'history' ? 'active' : ''}`} onClick={() => setTab('history')} onKeyDown={event => handleTabKeyDown(event, 'history')}>Class history ({bookings.length})</button>{managerData && <button ref={packagesTabRef} id="student-packages-tab" type="button" role="tab" tabIndex={tab === 'packages' ? 0 : -1} aria-selected={tab === 'packages'} aria-controls="student-packages-panel" className={`flex-1 ${tab === 'packages' ? 'active' : ''}`} onClick={() => setTab('packages')} onKeyDown={event => handleTabKeyDown(event, 'packages')}>Packages ({packages.length})</button>}</div>{tab === 'history' || !managerData ? <div id="student-history-panel" role="tabpanel" tabIndex={0} aria-labelledby="student-history-tab">{bookings.length ? <div className="max-h-80 space-y-3 overflow-y-auto pr-1">{bookings.map(booking => { const participant = booking.participants.find(item => item.studentId === student.id)!; return <article key={booking.id} className="rounded-xl border border-[#e6eae3] p-3"><div className="flex flex-wrap justify-between gap-2"><h3 className="text-xs text-[#344b39]">{booking.serviceName}</h3><span className={`badge ${booking.status.toLowerCase()}`}>{booking.status.toLowerCase()}</span></div><p className="mt-2 text-[11px] text-stone-500">{shortDate(booking.startAt)} · {time(booking.startAt)} · {booking.locationName}</p><div className="mt-2 flex flex-wrap gap-2 text-[10px] text-[#59675c]"><span>{booking.instructorName}</span><span>·</span><span>{participant.attendance === 'PRESENT' ? 'Attended' : participant.attendance === 'ABSENT' ? 'Absent' : 'Attendance not marked'}</span>{managerData && <ParticipantFinancialSummary data={managerData} bookingId={booking.id} studentId={student.id} />}</div></article>; })}</div> : <Empty title="No classes yet" icon={CalendarDays}>Bookings will appear here once this student joins a class.</Empty>}</div> : <div id="student-packages-panel" role="tabpanel" tabIndex={0} aria-labelledby="student-packages-tab">{packages.length ? <div className="space-y-3">{packages.map(pkg => <StudentPackageSummary key={pkg.id} pkg={pkg} data={managerData} />)}</div> : <Empty title="A fresh set of credits awaits" icon={Package}>Sell a package to give this student a bundle of classes or rentals.</Empty>}</div>}</div></DialogContent></Dialog>;
}
