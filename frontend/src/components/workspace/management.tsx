  ...
  return <>
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      <section className="panel overflow-hidden">
        ...
        <div className="overflow-x-auto px-4 pb-5 sm:px-5" tabIndex={0} role="region" aria-label="Weekly receipts chart"><div className="min-w-[390px] sm:min-w-0"><div className="flex h-52 items-stretch gap-2" role="img" aria-label={`Weekly recorded receipts total ${money(revenue)}. Exact amounts are in the weekly summary below.`}>{weeks.map((week, index) => <div key={week.start} className="flex min-w-0 flex-1 flex-col justify-end text-center"><span className="mb-2 truncate text-[9px] text-stone-500" title={money(week.revenue)}>{money(week.revenue)}</span><div className="flex h-36 items-end justify-center rounded-t-md bg-[#fafbf8]"><div title={`${shortDate(week.start + 'T12:00:00+08:00')}: ${money(week.revenue)}`} className={`w-full max-w-12 rounded-t-md ${index === weeks.length - 1 ? 'bg-[#52764b]' : 'bg-[#dbe5cf]'}`} style={{ height: `${week.revenue ? Math.max(2, week.revenue / maxRevenue * 100) : 0}%` }} /></div><span className="mt-2 text-[9px] text-[#59675c]">{week.start.slice(8)}/{week.start.slice(5, 7)}</span></div>)}</div></div><p className="mt-4 text-[10px] leading-relaxed text-[#59675c]">Weeks start on Monday. The current week is incomplete. Receipts include packages and unallocated payments; they are not recognised lesson revenue.</p></div>
      </section>
      <section className="panel overflow-hidden">
        ...
        <div className="overflow-x-auto px-4 pb-5 sm:px-5" tabIndex={0} role="region" aria-label="Weekly attendance chart"><div className="min-w-[390px] sm:min-w-0"><div className="flex h-52 items-stretch gap-2" role="img" aria-label={`Attendance: ${attended} present, ${absent} absent, ${unmarked} unmarked. Exact counts are in the weekly summary below.`}>{weeks.map(week => <div key={week.start} className="flex min-w-0 flex-1 flex-col justify-end text-center"><span className="mb-2 text-[9px] text-stone-500">{week.present + week.absent + week.unmarked}</span><div className="flex h-36 items-end justify-center rounded-t-md bg-[#fafbf8]"><div className="flex w-full max-w-12 flex-col-reverse overflow-hidden rounded-t-md" style={{ height: `${(week.present + week.absent + week.unmarked) / maxAttendance * 100}%` }}>{week.present > 0 && <div className="bg-[#779667]" style={{ flex: week.present }} />}{week.absent > 0 && <div className="bg-[#c6ab80]" style={{ flex: week.absent }} />}{week.unmarked > 0 && <div className="bg-[#e6eae1]" style={{ flex: week.unmarked }} />}</div></div><span className="mt-2 text-[9px] text-[#59675c]">{week.start.slice(8)}/{week.start.slice(5, 7)}</span></div>)}</div></div><div className="mt-4 flex flex-wrap gap-4 text-[10px] text-stone-500"><span className="flex items-center gap-1.5"><i aria-hidden="true" className="h-2 w-2 rounded-sm bg-[#779667]" />Present</span><span className="flex items-center gap-1.5"><i aria-hidden="true" className="h-2 w-2 rounded-sm bg-[#c6ab80]" />Absent</span><span className="flex items-center gap-1.5"><i aria-hidden="true" className="h-2 w-2 rounded-sm bg-[#e6eae1]" />Unmarked ({unmarked})</span></div><p className="mt-3 text-[10px] leading-relaxed text-[#59675c]">The attendance rate excludes unmarked participants. Future lessons and cancellations are excluded from this chart.</p></div>
      </section>
    </div>
    <section className="panel mt-5 overflow-hidden">
      <div className="panel-heading flex-wrap"><h2 className="text-[#294735]">Weekly summary</h2><span className="text-[10px] text-[#59675c]">Exact values behind the charts</span></div>
      <div className="table-wrap max-sm:hidden"><table className="data-table"><thead><tr><th>Week beginning</th><th>Receipts</th><th>Elapsed lessons</th><th>Present</th><th>Absent</th><th>Unmarked</th></tr></thead><tbody>{reversedWeeks.map(week => <tr key={week.start}><td>{shortDate(week.start + 'T12:00:00+08:00')}{week.start === thisMonday && <span className="ml-2 text-[9px] text-[#59675c]">In progress</span>}</td><td className="font-medium text-[#254b38]">{money(week.revenue)}</td><td>{week.sessions}</td><td>{week.present}</td><td>{week.absent}</td><td className="text-[#59675c]">{week.unmarked}</td></tr>)}</tbody></table></div>
      <div className="divide-y divide-[#edf0e8] sm:hidden">{reversedWeeks.map(week => <article key={week.start} className="p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="text-xs text-[#344b39]">Week of {shortDate(week.start + 'T12:00:00+08:00')}</h3>{week.start === thisMonday && <p className="mt-1 text-[10px] text-[#59675c]">In progress</p>}</div><strong className="text-sm text-[#254b38]">{money(week.revenue)}</strong></div><dl className="mt-3 grid grid-cols-4 gap-2 text-center"><div className="rounded-lg bg-[#f7f8f5] p-2"><dt className="text-[9px] text-[#59675c]">Lessons</dt><dd className="mt-1 text-xs font-semibold">{week.sessions}</dd></div><div className="rounded-lg bg-[#f1f5ed] p-2"><dt className="text-[9px] text-[#59675c]">Present</dt><dd className="mt-1 text-xs font-semibold">{week.present}</dd></div><div className="rounded-lg bg-[#faf6ed] p-2"><dt className="text-[9px] text-[#59675c]">Absent</dt><dd className="mt-1 text-xs font-semibold">{week.absent}</dd></div><div className="rounded-lg bg-[#f7f8f5] p-2"><dt className="text-[9px] text-[#59675c]">Unmarked</dt><dd className="mt-1 text-xs font-semibold">{week.unmarked}</dd></div></dl></article>)}</div>
    </section>
    <section className="panel mt-5 overflow-hidden">
      ...
      <p className="border-t border-[#edf0e8] px-4 py-4 text-[10px] leading-relaxed text-[#59675c] sm:px-6">Linked receipts are payments attached directly to these lessons, regardless of receipt date. Package purchases and unallocated payments cannot be attributed to a service and are excluded from this table.</p>
    </section>
  </>;
}
