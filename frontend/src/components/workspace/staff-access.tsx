...
type StaffUser = {
  ...
  name: string;
  email: string;
  accountType: 'OWNER' | 'COACH' | 'CUSTOMER';
...
type MembershipWorkspace = ManagementProps['data'] & { membership?: { role: StaffUser['role'] } | null };
const activeRole = (data: ManagementProps['data']) => (data as MembershipWorkspace).membership?.role || data.user.role;
...
export function StaffAccess(props: ManagementProps) {
  ...
  return activeRole(props.data) === 'OWNER' ? <OwnerStaffAccess key={`${props.data.business.id}-${props.data.user.id}`} {...props} /> : null;
}
...
function OwnerStaffAccess({ data, refresh }: ManagementProps) {
  ...
  return <section className="mt-10 border-t border-[#e6eae3] pt-8" aria-labelledby="staff-access-heading">
    <div className="mb-5 flex flex-wrap items-start justify-between gap-4">
      <div><p className="eyebrow mb-2">Owner controls</p><h2 id="staff-access-heading" className="text-xl">Staff workspace access</h2><p className="mt-2 max-w-xl text-xs leading-relaxed text-stone-500">Connect a coach or administrator who already has their own Courtly account.</p></div>
      <Button onClick={() => setEditing(null)} disabled={loading || !!error} className="max-sm:w-full"><Plus size={15} />Add staff member</Button>
    </div>
    <div className="mb-5 flex items-start gap-3 rounded-xl border border-[#e3e9db] bg-[#eff3e9] p-4">
      ...
      <div className="space-y-1.5 text-xs leading-relaxed text-[#5c7054]"><p><strong className="font-semibold">Accounts stay personal.</strong> Ask the staff member to register first, then enter the same email they used for Courtly. You never create or handle their password.</p><p>Removing access only unlinks this business. Their global account, other club memberships, instructor roster profile, and lesson history remain intact.</p></div>
    </div>
    <div className="panel overflow-hidden" aria-busy={loading}>
      {loading ? <p role="status" className="flex items-center justify-center gap-2 p-8 text-xs text-stone-500"><Loader2 size={16} className="animate-spin" />Loading staff access…</p> : error ? <div className="space-y-3 p-5"><p role="alert" className="text-xs text-red-700">{error}</p><Button variant="outline" size="sm" onClick={() => { void loadStaff().catch(() => {}); }}>Try again</Button></div> : staff.length ? <ul className="divide-y divide-[#edf0e8]">
        {staff.map(user => {
          const owner = user.role === 'OWNER' || user.userId === data.user.id;
          ...
          return <li key={user.id} className="flex flex-wrap items-center justify-between gap-4 p-5">
            <div className="flex min-w-0 flex-1 items-start gap-3">
              ...
              <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="break-words text-sm">{user.name}</h3><span className="badge">{roleLabels[user.role]}</span>{user.userId === data.user.id && <span className="text-[10px] text-stone-400">You</span>}</div><p className="mt-1 break-all text-xs text-stone-500">{user.email}</p><p className="mt-2 text-[11px] text-stone-500">{instructor ? `Instructor: ${instructor.name}${instructor.active ? '' : ' (archived)'}` : user.instructorId ? 'Linked instructor unavailable' : user.role === 'COACH' ? 'Coach roster profile is being prepared' : 'Administrator access'}</p></div>
            </div>
            {owner ? <span className="flex items-center gap-1.5 text-[11px] text-[#7b867d]"><LockKeyhole size={13} />Protected owner access</span> : <div className="flex gap-2 max-sm:ml-13"><Button variant="outline" size="sm" onClick={() => setEditing(user)} aria-label={`Edit access for ${user.name}`}><Pencil size={13} />Edit</Button><Button variant="destructive" size="sm" onClick={() => setRemoving(user)} aria-label={`Remove ${user.name} from this business`}><Unlink size={13} />Remove</Button></div>}
          </li>;
        })}
      </ul> : <Empty title="Just you for now" icon={ShieldCheck}>Add a registered Courtly account when someone on your team needs access.</Empty>}
    </div>
...
function StaffEditor({ staffUser, staff, data, refresh, onSaved, onClose }: ManagementProps & { staffUser: StaffUser | null; staff: StaffUser[]; onSaved: (user: StaffUser) => void; onClose: () => void }) {
  ...
  return <Editor title={staffUser ? 'Edit staff access' : 'Add a staff member'} description={staffUser ? 'Change this membership’s role or roster link. Their global profile and login stay unchanged.' : 'Enter the email of an existing Courtly account. If they have not registered yet, ask them to create a coach account first.'} onClose={onClose} refresh={refresh} success={staffUser ? 'Staff access updated' : 'Staff member added'} submitLabel={staffUser ? 'Save staff access' : 'Add to this business'} onSubmit={async form => {
    const values = { role, instructorId: role === 'COACH' ? instructorId || null : null };
    ...
    <div className="form-grid max-sm:grid-cols-1!">
      ...
      {role === 'COACH' && <Field label="Instructor profile (optional)" name="staff-instructor"><select id="staff-instructor" value={instructorId} onChange={event => setInstructorId(event.target.value)}><option value="">Create a roster profile automatically</option>{data.instructors.map(instructor => {
    ...
    <div className="space-y-2 rounded-lg border border-[#e3e9db] bg-[#f6f8f2] p-3 text-[11px] leading-relaxed text-[#64765d]"><p>{role === 'ADMIN' ? 'Admins manage day-to-day workspace operations. Only the owner can manage staff access.' : 'Coaches see their linked schedule. Choose an existing unclaimed roster profile, or Courtly will create one from their account.'}</p><p>This grants access to this business only. Their password and other memberships remain private.</p></div>
  </Editor>;
}
...
function RemoveStaff({ staffUser, refresh, onRemoved, onClose }: { staffUser: StaffUser; refresh: () => Promise<void>; onRemoved: () => void; onClose: () => void }) {
  ...
  async function remove() {
    ...
    try {
      ...
      onRemoved();
      toast.success('Staff member removed from this business.');
